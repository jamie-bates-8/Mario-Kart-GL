...
//	private static Model grass_top_lower_model = OBJParser.parseIndexedArrays("grass_top_lower", false, true,  new Vec2(2.5, 2).multiply(1.5f), new Vec2(10, 1));
	...
	static Model grass_top_upper_model = OBJParser.parseTexturedTriangleMesh("grass_top_upper_2", "grass_top_upper_3", new Vec2(1.5, 1).multiply(1.5f), new Vec2(8, 1));
	static Model grass_top_lower_model = OBJParser.parseTexturedTriangleMesh("grass_top_lower_3", "grass_top_lower_4", new Vec2(2.5, 2).multiply(1.5f), new Vec2(10, 1));
...
//	private static Model rock_wall_lower_model = new Model("rock_wall_lower");
	...
	public static Material grass_top_mat;
	...
	public GreenFort(GL2 gl)
	{
		...
		block_corner = new CheckeredBlock(gl, BlockType.BLOCK, CheckeredBlock.BLOCK_GREEN, new Vec3(33.75, 0, 33.75), new Vec3(0, 90, 0));
		...
		rock_wall_upper_node.useSpecularMap(false);
		rock_wall_upper_node.setScale(new Vec3(15));
		rock_wall_upper_node.setTranslation(new Vec3(90, 1, 90));
		...
		rock_wall_lower_node.useSpecularMap(false);
	...
	public void render(GL2 gl)
	{
		...
		if(displayModel)
		{
			...
			if(!Scene.normalMode && Scene.enableInstanced) WoodBridge.renderSimplified(gl);
			else
			{
...
//			Scene.renderQuery.begin(gl);
			...
			if(Scene.enableInstanced) CheckeredBlock.renderSimplified(gl);
			else
			{
