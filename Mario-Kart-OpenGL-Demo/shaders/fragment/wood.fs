...
vec3 getWoodColor()
{
	...
	noiseVec.x = snoise(-vertexPosition.x * noiseScale) * noisiness;
	noiseVec.y = snoise(-vertexPosition.y * noiseScale) * noisiness;
	noiseVec.z = snoise(-vertexPosition.z * noiseScale) * noisiness;
	
	noiseVec = normalize(noiseVec) * 0.1;
	
	vec3 location = -vertexPosition + noiseVec;
	...
	vec3 color = mix(lightColor, darkColor, r);
	
	r = fract((-vertexPosition.x + -vertexPosition.z) * grainScale + 0.5);
	noiseVec.z *= r;

	//if (r < grainThreshold) color += lightColor * lightGrains * noiseVec.z;
	//else                    color -= lightColor *  darkGrains * noiseVec.z;
...
{ 
    ...
    vec4 textureColor = vec4(getWoodColor(), 1.0);
	...
	gl_FragData[0] = vec4(getWoodColor(), 1.0);
