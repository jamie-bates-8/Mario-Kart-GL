...
void main(void)
{
    ...
    vec2 reflectCoord = (fragCoord.st / fragCoord.q) * 0.5 + 0.5;
    reflectCoord = clamp(reflectCoord, 0.002, 0.998);
             						         
	vec4 linearColor = shadowCoefficient * (ambient + diffuse + specular) * gl_Color;
	linearColor = mix(texture2D(reflectSampler, reflectCoord.st), linearColor, 0.9);
