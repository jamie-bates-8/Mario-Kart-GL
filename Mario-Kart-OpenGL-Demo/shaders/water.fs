...
void main() {
    ...
    vec3 specColor = mix(vec3(1.0,0.5,0.2), vec3(1.0,1.0,1.0), clamp(1.0-exp(-(sunPos.z*2.0)* sunext),0.0,1.0));
    ...
    color = (cameraPos.z<0.0)?mix(clamp(refraction*1.5,0.0,1.0),reflection,fresnel):color;   
